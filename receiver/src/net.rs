...
use common::packet::{now_us, AudioPacket, MAX_FRAMES, MAX_PACKET_BYTES};
use common::protocol::{
    ClientHello, HandshakeResult, HostMessage, ReceiverMessage, ServerHello, PROTOCOL_VERSION,
};
...
async fn run_session(
    ...
    if server_hello.version != PROTOCOL_VERSION {
        return Err(format!(
            "host version {} — receiver expects {PROTOCOL_VERSION}",
    ...
    // Bind forward audio UDP + query output device sample rate
    ...
    // Start forward audio playback (host mic → output device)
    ...
    // Spawn return stream task (DAW loopback → host)
    let return_host_addr = format!("{host}:{}", server_hello.return_udp_port);
    let return_task = tokio::spawn(return_stream_task(return_host_addr));
    ...
    loop {
        if DISCONNECT_REQUESTED.swap(false, Ordering::AcqRel) {
            eprintln!("[receiver] disconnect requested");
        ...
        tokio::select! {
            ...
            maybe_msg = tcp_in_rx.recv() => {
                match maybe_msg {
                    None => { eprintln!("[receiver] TCP reader closed"); break; }
                    Some(Err(e)) => return Err(e.into()),
                    Some(Ok(msg)) => match msg {
                        HostMessage::Pong { timestamp_us, .. } => {
                            let one_way_us = (now_us().saturating_sub(timestamp_us) / 2) as u32;
                        ...
                        HostMessage::Error(e) => return Err(format!("host error: {e}").into()),
                    }
                }
            }
            ...
            result = udp.recv(&mut udp_buf) => {
                let n = result?;
                if let Ok(pkt) = AudioPacket::decode(&udp_buf[..n]) {
...
// ---------------------------------------------------------------------------
// Return stream task — captures DAW loopback, sends to host
// ---------------------------------------------------------------------------

async fn return_stream_task(host_addr: String) {
    ...
    loop {
        ...
        if let Some(ref mut rx) = capture_rx {
            match rx.recv().await {
                ...
                None => {
                    eprintln!("[return] capture device closed");
                    drop(capture_shutdown.take());
                    capture_rx = None;
